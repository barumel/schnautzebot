...
function Widget(config, bot) {
  ...
  const commands = {
    ...
    '--play': function(discordChannelId, args) {
      const { url, target } = args;
      play(url, target, discordChannelId);
    }
  };
  ...
  async function unsubscribeChannel(url, target) {

  }

  async function play(url, target, discordChannelId) {
    if (isStreaming) return;
    isStreaming = true;

    const channel = bot.getChannelByName(target);
    if (!channel) bot.sendMessage(discordChannelId, `Channel with name ${target} not found!`);

    await bot.joinVoiceChannel(channel.id);
    const stream = await bot.getAudioContext(channel.id);

    const youtubeStream =  ytdl(url, { filter: 'audioonly' })
